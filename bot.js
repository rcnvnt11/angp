 ...
 * Fokus perubahan: robust terhadap 429/rate-limit & anti-crash.
 */
...
const PAY_COINS = (process.env.DETECT_PAY_COINS || SUI).split(',').map(s => s.trim()).filter(Boolean);
...
const DEX_NAME_LIST = (process.env.DEX_NAME_LIST||'').split(',').map(s=>s.trim().toLowerCase()).filter(Boolean);
...
const DEFAULT_EXCLUDE_PACKAGES = [
  ...
  '0x8a6b0ec259fbfa327656a5930ba7e2eb74e070402974e3711b2a3b761d39db48'
].join(',');
const EXCLUDE_PACKAGES = (process.env.EXCLUDE_PACKAGES || DEFAULT_EXCLUDE_PACKAGES)
  .split(',').map(s=>s.trim().toLowerCase()).filter(Boolean);

// FlowX package default — tetap
...
// === Fair-launch / aggregator yg dihindari pada rute ===
const FAIRLAUNCH_PROTOCOLS = ['HopFun','MovePump','DoubleUpPump','TurbosFun','BlastFun'];

// === Anti-LP: dari NAMA FUNGSI saja (swap tidak ikut ke-blok) ===
...
// ===== Client (proxy auto-retry 429/503/timeout) & Keys =====
const _rawClient = new SuiClient({ url: HTTP_URL });

function _isRetryable(e){
  const msg = String(e?.message || e || '').toLowerCase();
  return /429|too many|503|timeout|temporar|rate|thrott/.test(msg);
}
async function _rpcRetry(fn, label='rpc', max=5){
  let attempt = 0, lastErr;
  while (attempt < max){
    try { return await fn(); }
    catch (e){
      lastErr = e;
      if (!_isRetryable(e)) throw e;
      const base = 200 * Math.pow(2, attempt);     // 200,400,800,1600,3200
      const jitter = Math.floor(Math.random()*120);
      const wait = Math.min(3000, base + jitter);  // cap 3s
      if (!QUIET_429) console.log(`[RATE-LIMIT] ${label} ${attempt+1}/${max} → retry ~${wait}ms`);
...
const client = new Proxy(_rawClient, {
  get(target, prop){
    ...
    return async (...args)=> _rpcRetry(()=> val.apply(target, args), `client.${String(prop)}`);
  }
});
...
async function menu(){
  ...
  while(true){
    let suiBal = '(rate-limited)';
    try { suiBal = fmtMist(await getBalanceRaw(OWNER, SUI)); }
    catch(e){ if (!QUIET_429) console.log('[WARN] getBalance rate-limited:', String(e?.message||e)); }
    ...
    if(action==='set_tp'){
      ...
      await saveTokens();
      console.log(`[TP SET] ${key} target=${upd.tpPriceSui} SUI per 1 token, sel%=${upd.tpSellPercent}%`);
    }
